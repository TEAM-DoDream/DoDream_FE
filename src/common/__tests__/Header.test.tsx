...
jest.mock('../Header', () => {
  return function MockHeader({ type }: { type: 'show' | 'hide' }) {
    ...
    const profileImage = useUserStoreModule.useUserStore((state) => state.userImage);
    ...
    return (
      <div data-testid="header">
        ...
        <nav>
          ...
          <a 
            data-testid="nav-academy-info" 
            className={`text-gray-600`}
          >
            학원 정보
          </a>
          <a 
            data-testid="nav-recruit-info" 
            className={`text-gray-600`}
          >
            채용 정보
          </a>
...
describe('Header 컴포넌트', () => {
  ...
  beforeEach(() => {
    ...
    // useUserStore 스파이 설정
    jest.spyOn(useUserStoreModule, 'useUserStore').mockImplementation((selector) => {
